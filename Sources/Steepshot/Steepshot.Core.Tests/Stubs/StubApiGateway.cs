...
namespace Steepshot.Core.Tests.Stubs
{
    ...
    {
        protected override async Task<OperationResult<T>> CreateResult<T>(HttpResponseMessage response, Func<DounloadModel, Task<T>> readDelegate, CancellationToken ct)
        {
            ...
            return await base.CreateResult<T>(response, readDelegate, ct);
        }
