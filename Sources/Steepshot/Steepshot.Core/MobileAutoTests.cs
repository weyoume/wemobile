...
namespace Steepshot.Core
{
    public class MobileAutoTests
    {
        ...
        private void VoteTest(StringBuilder sb, int num)
        {
            ...
            var getPosts = new PostsRequest(PostType.New) { Login = _user.Login };
            Post testPost;
            do
            {
            ...
            var votereq = new VoteRequest(_user, VoteType.Up, testPost.Url);
        ...
        private void FollowTest(StringBuilder sb, int num)
        {
            ...
            var votereq = new FollowRequest(_user, FollowType.Follow, testPost.Author);
            ...
            var userFriendsReq = new UserFriendsRequest(_user.Login, FriendsType.Followers) { Login = _user.Login, Offset = testPost.Author, Limit = 1 };
        ...
        private void CreateCommentTest(StringBuilder sb, int num)
        {
            ...
            var getComm = new NamedInfoRequest(testPost.Url) { Offset = rez.Result.Permlink, Limit = 1 };
        ...
        private void UploadTest(StringBuilder sb, int num)
        {
            ...
            var request = new UploadImageRequest(_user, "Upload test", cat, new[] { "spam" });
            ...
            var response = _api.CreatePost(request, serverResp.Result, CancellationToken.None).Result;
            if (!response.IsSuccess)
            {
                sb.AppendLine($"fail. Reason:{Environment.NewLine} {response.Error.Message}");
                return;
        ...
        private void GetCommentsTest(StringBuilder sb, int num)
        {
            ...
            var request = new NamedInfoRequest(testPost.Url) { Limit = limit };
        ...
        private void GetUserProfileTest(StringBuilder sb, int num)
        {
            ...
            var request = new UserProfileRequest(_user.Login);
        ...
        private void GetUserFriendsTest(StringBuilder sb, int num)
        {
            ...
            var request = new UserFriendsRequest(_user.Login, FriendsType.Followers) { Limit = limit };
            ...
            request = new UserFriendsRequest(_user.Login, FriendsType.Following) { Limit = limit };
            response = _api.GetUserFriends(request, CancellationToken.None).Result;
        ...
        private void GetPostInfoTest(StringBuilder sb, int num)
        {
            ...
            var getPosts = new PostsRequest(PostType.Top);
            ...
            var request = new NamedInfoRequest(testPost.Url);
        ...
        private void SearchUserTest(StringBuilder sb, int num)
        {
            ...
            var request = new SearchWithQueryRequest(_user.Login) { Limit = limit };
        ...
        private void UserExistsCheckTest(StringBuilder sb, int num)
        {
            ...
            var request = new UserExistsRequests(_user.Login);
        ...
        private void GetCategoriesTest(StringBuilder sb, int num)
        {
            ...
            var request = new OffsetLimitFields { Limit = limit };
        ...
        private void SearchCategoriesTest(StringBuilder sb, int num)
        {
            ...
            var request = new SearchWithQueryRequest("go") { Limit = limit };
        ...
        {
            var getPosts = new PostsRequest(postType);
            Post testPost;
            do
            {
                var postsResp = _api.GetPosts(getPosts, CancellationToken.None)
